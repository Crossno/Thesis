...
\section{Calibration}
\newthought{Circuit losses and couplings are difficult to calculate a priori}, and while $T_n$ can be modulated away in a differential measurement, the generalize gain $\mathcal{G}(\Gamma)$ must be calibrated. If the output voltage is written in the form of eq.~\ref{eq:power_output} then $\mathcal{G}$ is given by:
\begin{equation}
